...
jalali_get_date (int p, struct jtm *j)
{
  ...
  while (1)
	...
	JALALI_LEAP_YEAR_LENGTH_IN_DAYS : JALALI_NORMAL_YEAR_LENGTH_IN_DAYS;
 ...
 */
int
jalali_year_month_days (int year, int month)
{
...
jalali_show_time (const struct jtm *j)
{
  ...
  printf (" yday: %d, dst: %d, off: %ld, zone: %s\n",
	  j->tm_yday, j->tm_isdst, j->tm_gmtoff, j->tm_zone);
}
...
int
compute_jdn (int year, int month, int day)
{
...
int
is_valid_jalali (int year, int month, int day)
{
...
int
is_valid_gregorian (int year, int month, int day)
{
