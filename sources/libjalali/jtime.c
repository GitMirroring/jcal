...
const char *fa_jalali_days_3[] =
  { "شنب", "یکش", "دوش", "سهش", "چها", "پنج",
  "جمع"
};
const char *jalali_days_2_fa[] = { "Sh", "Ye", "Do", "Se", "Ch", "Pa", "Jo" };

const char *fa_jalali_days_2[] =
...
const char *tzname_fa[2] =
  { "زمان زمستانی", "زمان تابستانی" };
	  ...
	  switch (format[i + 1])
	    ...
	    case 'c':
	      ...
	      snprintf (buf, MAX_BUF_SIZE, "%s %d %s %d %02d:%02d:%02d %s",
			...
			jtm->tm_hour, jtm->tm_min, jtm->tm_sec, jtm->tm_zone);
	      break;
 ...
 */
int
jalali_to_farsi (char *buf, size_t n, int padding, char *pad, int d)
{
  char _buf[100] = { 0 };
  int i = 0, j = 0;
  int p = 0;
