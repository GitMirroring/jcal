 ...
 * along with jcal.      If not, see <https://www.gnu.org/licenses/>.
 */
...
void
set_cal_matrix (struct cal_layout *l,
		struct jtm *ct, struct cal_matrix *mat, int prefix)
{
  struct jtm mb;
  struct jtm lt;
  ...
  for (i = 0, c = 0; i <= mat->height && c < m; i++)
      ...
      for (; j <= 6 && c < m; j++)
	{
	  if ((mb.tm_year == lt.tm_year) && (mb.tm_mon == lt.tm_mon)
	    ...
	    {
	      mat->m[i][j + _prefix] =
		(l->julian) ? 1000 + mb.tm_yday + c + 1 : 1000 + mb.tm_mday +
		c;
	    }
	  else
	    {
      ...
      for (j = 0; j < mat->width; j++)
	{
	  ...
	  if (m == 1)
	      ...
	      else if (mat->m[i][j] > 1000 && l->color)
		{
		  ...
		  printf ("%s%s%s ", TERM_BLACK_ON_WHITE, buf, TERM_RESET);
	      ...
	      else
		{
		  if (l->farsi)
		    ...
		    {
		      snprintf (buf, 100, "%*d", 2 + l->julian, mat->m[i][j]);
		    }
	      ...
	      else if (mat->m[i][j] > 1000 && l->color)
		{
		  ...
		  printf ("%s%s%s", TERM_RED_ON_WHITE, buf, TERM_RESET);
	      ...
	      else if (l->color)
		{
		  ...
		  printf ("%s%s%s", TERM_RED, buf, TERM_RESET);
  ...
  for (i = 0; i < m->n; i++)
    {
      snprintf (cal_t[i], MAX_BUF_SIZE, "%s %s", (l->farsi) ?
		...
		jalali_months[_j[i]->tm_mon], (l->syear) ? cal_y[i] : "");
